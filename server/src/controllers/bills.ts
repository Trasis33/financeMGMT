...
import { AuthRequest, BillWithRelations, CreateBillParticipant, BillFilters } from '../types/bills';
...
export const billsController = {
  async createBill(req: AuthRequest, res: Response) {
    try {
      ...
      const totalShare = participants.reduce((sum: number, p: CreateBillParticipant) => sum + p.share, 0);
      ...
      const bill = await prisma.bill.create({
        data: {
          ...
          creatorId: userId,
          participants: {
            create: participants.map((p: CreateBillParticipant) => ({
              userId: p.userId,
              share: p.share,
            })),
          },
  ...
  async getBills(req: AuthRequest, res: Response) {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Unauthorized' });
      ...
      const { startDate, endDate, category, isRecurring, isPaid } = req.query as unknown as BillFilters;
      ...
      const filteredBills = isPaid !== undefined
        ? bills.filter((bill: BillWithRelations) => {
            const totalPaid = bill.payments.reduce((sum: number, p) => sum + p.amount, 0);
  ...
  async getBill(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
  ...
  async updateBill(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
      ...
      if (updates.participants) {
        const totalShare = updates.participants.reduce((sum: number, p: CreateBillParticipant) => sum + p.share, 0);
        if (Math.abs(totalShare - 1) > 0.0001) {
          return res.status(400).json({ error: 'Total share must equal 100%' });
        }
      }
      ...
      const bill = await prisma.bill.update({
        ...
        data: {
          ...
          ...(updates.recurringPeriod && { recurringPeriod: updates.recurringPeriod }),
          ...(updates.category && { category: updates.category }),
          ...(updates.notes !== undefined && { notes: updates.notes }),
          ...(updates.participants && {
            participants: {
              deleteMany: {},
              create: updates.participants.map((p: CreateBillParticipant) => ({
                userId: p.userId,
                share: p.share,
              })),
            },
  ...
  async deleteBill(req: AuthRequest, res: Response) {
    try {
      const { id } = req.params;
      const userId = req.user?.id;
  ...
  async addPayment(req: AuthRequest, res: Response) {
    try {
      ...
      const totalPaid = bill.payments.reduce((sum: number, p) => sum + p.amount, 0) + amount;
  ...
  async getBillStatistics(req: AuthRequest, res: Response) {
    try {
      const userId = req.user?.id;
      if (!userId) {
        return res.status(401).json({ error: 'Unauthorized' });
